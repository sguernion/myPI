...
print result
response = json.loads(api.call_api('type=command&param=getlightswitches'))
...
for switch in response['result']:
	...
	if exist.status_code == 200 :
		print 'update switch'
		put_url(s,dds_url+'/device/'+str(switch['idx']),datafull)
	else:
		print 'create switch'
		post_url(s,dds_url+'/devices/',data)

	response3 = json.loads(api.call_api('type=lightlog&idx='+str(switch['idx'])))
